...
function Validator(formSelector) {
  ...
  const sendPostRequest = async () => {
    ...
    for (const radioGroup of radioGroups) {
      ...
      let value = radioGroup.querySelector(`input[name="${radioGroup.getAttribute("name")}"]:checked`)?.value;
      data2[`${pNode.innerText}`] = value ? value : "";
    }
    //-----------get checkbox---------------------
    //  + get all checkbox group
    let checkboxGroups = formElement.querySelectorAll("[name][groupCheckboxRules]");
    ...
    for (const checkboxGroup of checkboxGroups) {
      ...
      data2[`${pNode.textContent}`] = value ? value : "";
    ...
    for (const input of inputs) {
      ...
      data2[`${pNode.innerText.trim()}`] = value ? value : "";
    }
    ...
    // add coookie
    orig_aid = get_cookie_name("orig_aid");
    fosp_aid = get_cookie_name("fosp_aid");
    ...
    postData("https://script.google.com/macros/s/AKfycbxfLP7Q43x5aWl6dVOfDObuxaBsTCol_xIC_fh2CLJZZnwAMkCZFuEfrMHnPbBenD8O/exec", data2);
...
postADP = async () => {
  ...
  data = {
    fosp_aid: fosp_aid,
    orig_aid: orig_aid,
    fosp_uid: "4c44fac0d7ab127a4d59a19202e56b77",
    myvne_user_id: "1085352353",
    source_type: 1,
    source_name: "https://vnexpress.net",
    campaign_name: "BoxSurvey_13benh_Viemxoang_GuiKhaoSat",
  ...
  axios({
    ...
    .then(function (response) {
      ...
      if (response.status === 201 || response.status === 200) {
        console.log("postADP success");
      } else {
        // not ok
        console.log("postADP fail");
      }
    })
    ...
    .then(function (response) {
      ...
      if (response.status === 201 || response.status === 200) {
        Swal.fire({
          ...
          html:
            "<div class='pop-area'><p>Cảm ơn bạn đã tham gia</p> </br> <p>Bạn có thể chia sẻ khảo sát này cho người khác</p> </br> " +
            '<div class="social-share-pop"> <p class="d-none">Chia sẻ</p> <img src="assets/images/facebook.png" class="fb" alt="" /> <img src="assets/images/twiter.png" class="twiter" alt="" /> <img src="assets/images/link.png" class="link-copy" alt="" /> </div></div>',
          focusConfirm: false,
...
const changePage = (type, targetSectionId, currentSectionId) => {
  ...
  if (type === "next") {
    ...
    console.log(currentSection, "current");
...
var controlButtonNav = (type, targetSectionId) => {
  btnNext = document.querySelector(".btn-submit");
  btnPre = document.querySelector(".btn-nav-back");
  btnSubmit = document.querySelector(".btn-nav-next");
  if (type === "next") {
    //Đã là next thì luôn hiện back
    btnPre.classList.remove("d-none");
    if (logNavi.length > 1) {
      // Đã chuyển page nên log dài hơn 1 nghĩa là ở section sau
      // tuy nhiên bên trong phải chia ra trường hợp là cuối hay là từ 2, section cuối đã phải khai báo cứng;
      if (targetSectionId == lastSection) {
        // cuối
    ...
    //-back thì auto tắt submit
    btnSubmit.classList.add("d-none");
    //-check phải section đầu ko để tắt nút back
    console.log(targetSectionId, firstSection);
    if (targetSectionId == firstSection) {
      // đây là đầu ẩn back đi
      console.log("ẩn back");
...
shareSocial = (currentUrl) => {
  ...
  limks = document.querySelectorAll(".link-copy");
