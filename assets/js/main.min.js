...
function Validator(formSelector) {
  ...
  /// Hàm check riêng cho số lần rút
  SoLanRutCheck = () => {
    inputSoLanRut = document.querySelector(".input-soLanRut");
    inputSoLanRut.addEventListener("input", ({ target: { value } }) => {
      group = document.querySelector(".soLanRut");
      formGroup = group.closest(".form-group");
      console.log(formGroup);
      mess = formGroup.querySelector(".form-message");
      const pattern = /^([1-9]|1[0-9]|20)$/;
      if (pattern.test(value)) {
        ...
        mess.innerText = "Số lần rút không hợp lệ";
        formGroup.classList.add("invalid");
        console.log(formGroup);
        listCurrentError.push(formGroup.id);
        group.closest(".card").classList.add("invalid");
...
const sendPostRequest = async () => {
  ...
  for (const radioGroup of radioGroups) {
    ...
    dataOfRadio = radioGroup.querySelector(`input[name="${radioGroup.getAttribute("name")}"]:checked`)?.value;
    if (dataOfRadio != "more-ans") {
      value = dataOfRadio;
    } else {
      let input_more_ans = radioGroup.querySelector(".more-ans-input");
  ...
  dataCheckingFormBanner = {
    campaign_name: "survey_baniv_doanhnghiep_20230414", //name của ca
    engagement_type: "1003",
    source_name: "Vnexpress", // Hệ thống tự push vào
    source_type: "internal",
    index_brand: "survey_baniv_doanhnghiep_20230414",
    index_industrial: "",
    index_category: "",
    index_topic: "",

    //Nhập thông tin của audience
    name: "", // nhập tên audience (option)
    phone: "",
    gender: "",
    age: "",
    birth_day: "",
  ...
  postData("https://script.google.com/macros/s/AKfycbxfLP7Q43x5aWl6dVOfDObuxaBsTCol_xIC_fh2CLJZZnwAMkCZFuEfrMHnPbBenD8O/exec", data2, dataCheckingFormBanner);
...
postADP = async () => {
  ...
  fosp_uid = get_cookie_name("fosp_uid");
  data = {
    fosp_aid: fosp_aid,
    orig_aid: orig_aid,
    fosp_uid: fosp_uid,
  ...
  axios({
    ...
    .then(function (response) {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        // dataLayer.push({
        //   event: "voteEvent",
        //   vote_category: "survey_baniv_doanhnghiep_20230414",
        //   vote_action: "survey_baniv_doanhnghiep_20230414",
        // });
